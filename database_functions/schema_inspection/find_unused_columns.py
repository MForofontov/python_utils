...
def find_unused_columns(
    ...
    if not isinstance(null_threshold, (int, float)):
        raise TypeError(f"null_threshold must be float, got {type(null_threshold).__name__}")
    if not 0 <= null_threshold <= 1:
        raise ValueError(f"null_threshold must be between 0 and 1, got {null_threshold}")
                ...
                if null_percentage >= null_threshold:
                    ...
                    if null_count < total_rows:
                        try:
                            ...
                            if non_null_count > 0:
                                col_info["cardinality_ratio"] = distinct_count / non_null_count
                        except Exception as e:
                            logger.debug(f"Could not get distinct count for {table_name}.{col.name}: {e}")
