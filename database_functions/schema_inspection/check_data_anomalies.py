...
from sqlalchemy import MetaData, select, func, Integer, Float, Numeric
...
def check_data_anomalies(
    ...
    if not isinstance(check_all_same, bool):
        raise TypeError(f"check_all_same must be bool, got {type(check_all_same).__name__}")
    if not isinstance(check_outliers, bool):
        raise TypeError(f"check_outliers must be bool, got {type(check_outliers).__name__}")
    if not isinstance(outlier_std_threshold, (int, float)):
        raise TypeError(f"outlier_std_threshold must be float, got {type(outlier_std_threshold).__name__}")
    ...
    for table_name in table_names:
        ...
        for col in table.columns:
            try:
                ...
                # Check 1: All NULL
                null_count_query = select(func.count()).select_from(table).where(col.is_(None))
                ...
                if check_outliers and isinstance(col.type, (Integer, Float, Numeric)):
                    try:
                        ...
                        ).select_from(table).where(col.is_not(None))
                        ...
                        if stats and stats[1]:  # If stddev exists
                            ...
                            if outlier_count > 0:
                                outlier_percentage = outlier_count / (total_rows - null_count)
                                
                                # Only report if significant
                                if outlier_percentage > 0.01:  # More than 1%
                                    severity = "high" if outlier_percentage > 0.1 else "medium"
                        ...
                        logger.debug(f"Error checking outliers for {table_name}.{col.name}: {e}")
                
            except Exception as e:
                logger.error(f"Error checking anomalies for {table_name}.{col.name}: {e}")
