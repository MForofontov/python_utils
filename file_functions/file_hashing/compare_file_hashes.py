...
def compare_file_hashes(
    ...
    if not isinstance(file1_path, (str, Path)):
        raise TypeError(f"file1_path must be a string or Path, got {type(file1_path).__name__}")
    if not isinstance(file2_path, (str, Path)):
        raise TypeError(f"file2_path must be a string or Path, got {type(file2_path).__name__}")
    if not isinstance(hash_algorithm, str):
        raise TypeError(f"hash_algorithm must be a string, got {type(hash_algorithm).__name__}")
    ...
    if hash_algorithm.lower() not in valid_algorithms:
        raise ValueError(f"hash_algorithm must be one of {valid_algorithms}, got '{hash_algorithm}'")
