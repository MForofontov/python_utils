...
def excel_to_parquet(
    ...
    *,
    ...
    try:
        ...
        header = [
            str(col) if col is not None else f"Column{i}"
        ...
        if row_count == 0:
            ...
            data_dict = {
                col: [row[i] for row in data_rows] for i, col in enumerate(header)
            }
            ...
            if type_inference:
                ...
                string_dict = {
                    col: [str(v) if v is not None else None for v in vals]
