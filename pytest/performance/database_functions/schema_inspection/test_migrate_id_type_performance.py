...
def test_migrate_id_type_performance_small_batch_size(
    ...
    assert elapsed_time < 10.0, (
        f"Migration with batch_size=100 took {elapsed_time:.2f}s, expected < 10s"
    )
    assert mem_used < 50.0, f"Memory usage {mem_used:.2f} MB exceeds limit of 50 MB"
...
def test_migrate_id_type_performance_batch_size_comparison(
    ...
    for batch_size in batch_sizes:
        ...
        # Print results
        print(
            f"Batch size {batch_size:>5}: {elapsed_time:>6.3f}s | "
    ...
    print(
        f"✓ Most memory efficient: batch_size={most_efficient_mem[0]} ({most_efficient_mem[1]['memory']:.2f} MB)"
    )

    # Assert all completed successfully
    for batch_size, stats in results.items():
        assert stats["time"] < 10.0, (
            f"Batch size {batch_size} took {stats['time']:.2f}s, expected < 10s"
        )
        assert stats["memory"] < 50.0, (
            f"Batch size {batch_size} used {stats['memory']:.2f} MB, expected < 50 MB"
        )
...
def test_migrate_id_type_performance_stress_test_1m_rows(
    ...
    with engine.begin() as conn:
        for batch_start in range(0, 400000, batch_size):
            ...
            addresses_data = [
                {"address_id": i, "user_id": f"user-{i}", "street": f"Street {i}"}
                for i in range(batch_start, min(batch_start + batch_size, 400000))
            ]
            ...
            logs_data = [
                {"log_id": i, "user_id": f"user-{i}", "action": f"Action{i % 100}"}
                for i in range(batch_start, min(batch_start + batch_size, 400000))
            ]
            conn.execute(logs.insert(), logs_data)

    print("  ✓ FK tables complete (2,000,000 rows total)")
    print("  📊 Total setup: 2.4M rows (400K users + 2M FK references)")
    ...
    print("  • 400K users (main table)")
    print("  • 2M FK references (5 tables × 400K rows)")
    ...
    with engine.begin() as conn:
        ...
        users_check = conn.execute(
            text("SELECT COUNT(*) FROM users WHERE id LIKE 'new-user-%'")
        ).scalar()
        assert users_check == 400000, (
            f"Expected all 400K users to have new IDs, got {users_check}"
        )

        # Check no old IDs remain
        old_users_check = conn.execute(
            text("SELECT COUNT(*) FROM users WHERE id LIKE 'user-%'")
        ).scalar()
        assert old_users_check == 0, (
            f"Found {old_users_check} users with old IDs, expected 0"
        )

        # Check FK tables - all user_id references should be updated
        orders_check = conn.execute(
            text("SELECT COUNT(*) FROM orders WHERE user_id LIKE 'new-user-%'")
        ).scalar()
        assert orders_check == 400000, (
            f"Expected all 400K orders to have new user_ids, got {orders_check}"
        )

        addresses_check = conn.execute(
            text("SELECT COUNT(*) FROM addresses WHERE user_id LIKE 'new-user-%'")
        ).scalar()
        assert addresses_check == 400000, (
            f"Expected all 400K addresses to have new user_ids, got {addresses_check}"
        )

        payments_check = conn.execute(
            text("SELECT COUNT(*) FROM payments WHERE user_id LIKE 'new-user-%'")
        ).scalar()
        assert payments_check == 400000, (
            f"Expected all 400K payments to have new user_ids, got {payments_check}"
        )

        sessions_check = conn.execute(
            text("SELECT COUNT(*) FROM sessions WHERE user_id LIKE 'new-user-%'")
        ).scalar()
        assert sessions_check == 400000, (
            f"Expected all 400K sessions to have new user_ids, got {sessions_check}"
        )

        logs_check = conn.execute(
            text("SELECT COUNT(*) FROM logs WHERE user_id LIKE 'new-user-%'")
        ).scalar()
        assert logs_check == 400000, (
            f"Expected all 400K logs to have new user_ids, got {logs_check}"
        )
        ...
        orphaned_addresses = conn.execute(
            text(
