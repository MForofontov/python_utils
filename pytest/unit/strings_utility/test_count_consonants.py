...
def test_all_consonants() -> None:
    ...
    assert (
        count_consonants("bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ") == 42
    ), "Failed on all consonants"
    # Test case 4: String with only consonants
    assert (
        count_consonants("bcdfghjklmnpqrstvwxyz") == 21
    ...
    assert (
        count_consonants("bbccddffgghhjjkkllmmnnppqqrrssttvvwwxxyyzz") == 42
...
def test_non_english_characters() -> None:
    ...
    assert (
        count_consonants("héllo wörld") == 7
