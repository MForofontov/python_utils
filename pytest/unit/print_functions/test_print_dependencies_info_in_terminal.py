import pytest
import sys
from print_functions.print_dependencies_info_in_terminal import print_dependencies_info_in_terminal
from unittest.mock import patch


def test_print_dependencies_info_in_terminal_installed(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    """
...
def test_print_dependencies_info_in_terminal_not_installed(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    """
...
def test_print_dependencies_info_in_terminal_separator_and_title(capsys: pytest.CaptureFixture[str]) -> None:
    """
    Test case 3: Prints separator and title.
    """
    with patch("print_functions.print_dependencies_info_in_terminal.version", lambda dep: "1.0.0"):
        print_dependencies_info_in_terminal(["pytest"])
        out = capsys.readouterr().out
