...
def test_connection_pool_manager_health_check() -> None:
    ...
    pool = ConnectionPoolManager(
        create_conn, max_connections=2, health_check_query="SELECT 1"
    )
...
def test_connection_pool_manager_no_health_check() -> None:
    ...
    pool = ConnectionPoolManager(
        create_conn, max_connections=2, health_check_query=None
    )
...
def test_connection_pool_manager_invalid_health_check_interval() -> None:
    ...
    # Act & Assert
    with pytest.raises(ValueError):
        ConnectionPoolManager(create_conn, max_connections=5, health_check_interval=0)
