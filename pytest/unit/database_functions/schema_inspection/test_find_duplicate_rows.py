...
from pyutils_collection.database_functions.schema_inspection import find_duplicate_rows
...
def test_find_duplicate_rows_identifies_duplicates(memory_engine) -> None:
    ...
    conn.execute(
        ...
        {"id": 1, "email": "john@example.com", "username": "john1"},
    )
    conn.execute(
        User.__table__.insert(),
        {"id": 2, "email": "john@example.com", "username": "john2"},
    )
    conn.execute(
        User.__table__.insert(),
        {"id": 3, "email": "jane@example.com", "username": "jane"},
    )
    conn.commit()
...
def test_find_duplicate_rows_multiple_columns(memory_engine) -> None:
    ...
    conn.execute(
        User.__table__.insert(),
        {"id": 2, "email": "john@example.com", "username": "john"},
    )
    conn.execute(
        User.__table__.insert(),
        {"id": 3, "email": "john@example.com", "username": "john2"},
    )
    conn.commit()
...
def test_find_duplicate_rows_min_duplicates_threshold(memory_engine) -> None:
    ...
    conn.execute(
        User.__table__.insert(),
        {"id": 3, "email": "john@example.com", "username": "john3"},
    )
    conn.commit()
...
def test_find_duplicate_rows_sorted_by_count(memory_engine) -> None:
    ...
    conn.execute(
        User.__table__.insert(), {"id": 1, "email": "a@example.com", "username": "a1"}
    )
    conn.execute(
        User.__table__.insert(), {"id": 2, "email": "a@example.com", "username": "a2"}
    )
    conn.execute(
        User.__table__.insert(), {"id": 3, "email": "b@example.com", "username": "b1"}
    )
    conn.execute(
        User.__table__.insert(), {"id": 4, "email": "b@example.com", "username": "b2"}
    )
    conn.execute(
        User.__table__.insert(), {"id": 5, "email": "b@example.com", "username": "b3"}
    )
    conn.commit()
...
def test_find_duplicate_rows_with_primary_key_sample_ids(memory_engine) -> None:
    ...
    conn.execute(
        User.__table__.insert(),
        {"id": 1, "email": "test@example.com", "username": "user1"},
    )
    conn.execute(
        User.__table__.insert(),
        {"id": 2, "email": "test@example.com", "username": "user2"},
    )
    conn.execute(
        User.__table__.insert(),
        {"id": 3, "email": "test@example.com", "username": "user3"},
    )
    conn.commit()
