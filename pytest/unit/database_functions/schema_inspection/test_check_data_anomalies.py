...
def test_check_data_anomalies_check_all_same_false(memory_engine) -> None:
    ...
    all_same_anomalies = [
        a for a in anomalies if "same" in a.get("anomaly_type", "").lower()
    ]
...
def test_check_data_anomalies_invalid_outlier_threshold_type_error(
    memory_engine,
) -> None:
    """
...
def test_check_data_anomalies_skips_primary_key_columns(memory_engine: Engine) -> None:
    ...
    with memory_engine.connect() as connection:
        anomalies = check_data_anomalies(
            connection, check_all_same=True, check_outliers=False
        )
    ...
    assert ("products", "quantity") in anomaly_columns, (
        "Quantity column should be flagged"
    )


def test_check_data_anomalies_outlier_detection_sqlite_limitation(
    memory_engine,
) -> None:
    """
