...
def test_suggest_data_type_optimizations_oversized_varchar(memory_engine) -> None:
    ...
    with memory_engine.connect() as connection:
        suggestions = suggest_data_type_optimizations(
            connection, tables=["products"], sample_size=100
        )
...
def test_suggest_data_type_optimizations_invalid_tables_type_error(
    memory_engine,
) -> None:
    """
...
def test_suggest_data_type_optimizations_invalid_schema_type_error(
    memory_engine,
) -> None:
    """
...
def test_suggest_data_type_optimizations_invalid_sample_size_type_error(
    memory_engine,
) -> None:
    """
...
def test_suggest_data_type_optimizations_negative_sample_size_value_error(
    memory_engine,
) -> None:
    """
...
def test_suggest_data_type_optimizations_includes_potential_savings(
    memory_engine,
) -> None:
    """
    ...
    with Session(memory_engine) as session:
        for i in range(100):
            session.add(
                Product(id=i, name="N", sku="X", description="Y", price=1.0, status="Z")
            )
        session.commit()
