...
from pyutils_collection.testing_functions.assertion_helpers.assert_raises_with_message import (
...
def test_assert_raises_with_message_assertion_error_no_exception() -> None:
    ...
    # Act & Assert
    with pytest.raises(
        AssertionError, match="Expected .* to be raised, but no exception was raised"
    ):
