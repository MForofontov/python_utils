...
def test_validate_email_case_8_value_error_length_violations() -> None:
    ...
    with pytest.raises(ValueError, match="email exceeds maximum length of 254 characters"):
        validate_email(long_email)
    
    # Test local part too long (over 64 characters)
    long_local = "a" * 65 + "@example.com"
    with pytest.raises(ValueError, match="email local part exceeds maximum length of 64 characters"):
        validate_email(long_local)
    
    # Test domain part too long (over 253 characters)
    long_domain = "user@" + "a" * 250 + ".com"
    with pytest.raises(ValueError, match="email domain part exceeds maximum length of 253 characters"):
...
def test_validate_email_case_10_value_error_invalid_format() -> None:
    ...
    with pytest.raises(ValueError, match="email domain cannot contain consecutive dots"):
        validate_email("user@example..com")
...
def test_validate_email_case_12_value_error_unicode_not_allowed() -> None:
    ...
    """
    with pytest.raises(ValueError, match="email contains non-ASCII characters but allow_unicode=False"):
