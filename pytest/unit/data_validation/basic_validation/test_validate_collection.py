...
def test_validate_collection_case_10_value_error_length_bounds() -> None:
    ...
    with pytest.raises(ValueError, match="collection length \\(1\\) is below minimum allowed length \\(3\\)"):
        validate_collection([1], list, min_length=3)
    
    # Test above maximum length
    with pytest.raises(ValueError, match="collection length \\(5\\) exceeds maximum allowed length \\(3\\)"):
        validate_collection([1, 2, 3, 4, 5], list, max_length=3)
    
    # Test with custom param name
    with pytest.raises(ValueError, match="items length \\(2\\) is below minimum allowed length \\(5\\)"):
...
def test_validate_collection_case_11_type_error_element_validation() -> None:
    ...
    with pytest.raises(TypeError, match="collection element at index 1 must be int, got str"):
        validate_collection([1, "hello", 3], list, element_type=int)
    
    # Test wrong union element type
    with pytest.raises(TypeError, match="collection element at index 2 must be int \\| str, got float"):
        validate_collection([1, "hello", 3.14], list, element_type=(int, str))
    
    # Test dict value type error
    with pytest.raises(TypeError, match="collection value for key 'b' must be int, got str"):
        validate_collection({"a": 1, "b": "hello"}, dict, element_type=int)
    
    # Test with custom param name
    with pytest.raises(TypeError, match="items element at index 0 must be str, got int"):
...
def test_validate_collection_case_12_type_error_invalid_parameters() -> None:
    ...
    with pytest.raises(ValueError, match="min_length \\(5\\) cannot be greater than max_length \\(3\\)"):
