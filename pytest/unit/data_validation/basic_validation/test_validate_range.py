...
def test_validate_range_case_9_invalid_range_bounds() -> None:
    ...
    """
    # Test min > max
    with pytest.raises(ValueError, match="min_value \\(10\\) cannot be greater than max_value \\(5\\)"):
        validate_range(7, min_value=10, max_value=5)
    
    # Test equal bounds with exclusive
    with pytest.raises(ValueError, match="min_value and max_value are equal \\(5\\), but at least one bound is exclusive"):
...
def test_validate_range_case_10_type_errors() -> None:
    ...
    with pytest.raises(TypeError, match="Cannot compare value of type int with min_value of type str"):
        validate_range(5, min_value="hello")
    
    with pytest.raises(TypeError, match="Cannot compare value of type str with max_value of type int"):
