...
def test_validate_string_value_error_length_violations() -> None:
    ...
    with pytest.raises(
        ValueError, match="value length \\(2\\) is below minimum allowed length \\(5\\)"
    ):
    ...
    with pytest.raises(
        ValueError,
        match="password length \\(3\\) is below minimum allowed length \\(8\\)",
    ):
...
def test_validate_string_value_error_pattern_mismatch() -> None:
    ...
    with pytest.raises(
        ValueError, match="value does not match required pattern: \\^\\[a-z\\]\\+\\$"
    ):
        validate_string("Hello123", pattern=r"^[a-z]+$")

    with pytest.raises(
        ValueError, match="value does not match required pattern: \\^\\\\d\\+\\$"
    ):
