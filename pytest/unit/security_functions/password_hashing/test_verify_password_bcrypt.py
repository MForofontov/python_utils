...
from security_functions.password_hashing.verify_password_bcrypt import verify_password_bcrypt
...
def test_verify_password_bcrypt_case_6_invalid_hash_format() -> None:
    ...
    with pytest.raises(ValueError, match="hashed_password is not a valid bcrypt hash format"):
        verify_password_bcrypt("password", "$1$invalid_hash_format_here")
    
    # Test wrong length
    with pytest.raises(ValueError, match="hashed_password must be exactly 60 characters"):
...
def test_verify_password_bcrypt_case_11_malformed_hash() -> None:
    ...
    with pytest.raises(ValueError, match="Invalid bcrypt hash format"):
        verify_password_bcrypt("password", "completely_invalid_hash_string_that_is_60_chars_long_x")
