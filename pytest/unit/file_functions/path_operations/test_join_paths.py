...
from python_utils.file_functions import join_paths


def test_join_paths_multiple_relative_segments(tmp_path: Path) -> None:
    """
    Test case 1: Test joining multiple relative child segments.
    """
...
def test_join_paths_empty_child_list_returns_parent(tmp_path: Path) -> None:
    """
    Test case 2: Test that empty child list returns the parent path unchanged.
    """
    parent_path: str = str(tmp_path)
...
def test_join_paths_absolute_child_supersedes_parent(tmp_path: Path) -> None:
    """
    Test case 3: Test that an absolute child path supersedes the parent path.
    """
...
def test_join_paths_non_string_child_raises_type_error(tmp_path: Path) -> None:
    """
    Test case 4: Test that providing non-string child paths raises a TypeError.
    """
    parent_path: str = str(tmp_path)
