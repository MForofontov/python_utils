...
def test_find_files_by_mtime_case_4_combined_filters() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Act
        result = find_files_by_mtime(temp_dir, newer_than=newer_than, older_than=older_than)
...
def test_find_files_by_mtime_case_5_no_criteria_error() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        # Act & Assert
        with pytest.raises(ValueError, match="At least one time criterion must be specified"):
...
def test_find_files_by_mtime_case_10_file_access_error_handling() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(Path, 'stat', mock_stat):
            # Act
            result = find_files_by_mtime(temp_dir, newer_than=datetime.now() - timedelta(days=1))
