...
def test_array_diff_basic() -> None:
    ...
    """
    assert np.array_equal(array_diff(np.array([1, 4, 9])), np.array([3, 5])), "Failed on increasing numbers"
...
def test_array_diff_single_element() -> None:
    ...
    """
    assert array_diff(np.array([42])).size == 0, "Failed on single element array"
