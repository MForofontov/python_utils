...
def test_array_cumsum_basic() -> None:
    ...
    """
    assert np.array_equal(array_cumsum(np.array([1, 2, 3])), np.array(
        [1, 3, 6])), "Failed on positive integers"
...
def test_array_cumsum_negative() -> None:
    ...
    """
    assert np.array_equal(array_cumsum(
        np.array([-1, -2, -3])), np.array([-1, -3, -6])), "Failed on negative numbers"
