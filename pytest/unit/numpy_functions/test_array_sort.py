...
def test_array_sort_basic() -> None:
    ...
    """
    assert np.array_equal(array_sort(np.array([3, 1, 2])), np.array([1, 2, 3])), "Failed on positive integers"
...
def test_array_sort_negative() -> None:
    ...
    """
    assert np.array_equal(array_sort(np.array([-3, -1, -2])), np.array([-3, -2, -1])), "Failed on negative numbers"
