...
from scientific_computing_functions.numerical_methods.solve_boundary_value_problem import solve_boundary_value_problem
...
def test_solve_bvp_linear_problem() -> None:
    ...
    # Act
    result = solve_boundary_value_problem(
        ode, bc, (0, 1), y_init, method='collocation'
    )
...
def test_solve_bvp_n_points_less_than_two() -> None:
    ...
    with pytest.raises(ValueError, match=expected_message):
        solve_boundary_value_problem(
            ode, bc, (0, 1), np.array([[0], [0]]), n_points=1
        )
...
def test_solve_bvp_negative_tolerance() -> None:
    ...
    with pytest.raises(ValueError, match=expected_message):
        solve_boundary_value_problem(
            ode, bc, (0, 1), np.array([[0], [0]]), tol=-1e-6
        )
...
def test_solve_bvp_zero_tolerance() -> None:
    ...
    with pytest.raises(ValueError, match=expected_message):
        solve_boundary_value_problem(
            ode, bc, (0, 1), np.array([[0], [0]]), tol=0.0
        )
...
def test_solve_bvp_invalid_method_type() -> None:
    ...
    with pytest.raises(TypeError, match=expected_message):
        solve_boundary_value_problem(
            ode, bc, (0, 1), np.array([[0], [0]]), method=123
        )
