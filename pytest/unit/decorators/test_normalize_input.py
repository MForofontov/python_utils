...
from python_utils.decorators.normalize_input import normalize_input
...
def test_normalize_input_with_variable_length_arguments() -> None:
    ...
    result = sample_function_args_kwargs(
        "hello", "world", "test", kwarg1="example", kwarg2="case"
    )
    assert (
        result
        == "args: ('HELLO', 'WORLD', 'TEST'), kwargs: {'kwarg1': 'EXAMPLE', 'kwarg2': 'CASE'}"
    )
...
def test_normalize_input_function_raises_error_with_logger(
...
def test_normalize_input_invalid_normalizer_with_logger(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """
...
def test_normalize_input_normalization_failure_with_logger(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """
    ...
    with caplog.at_level(logging.ERROR):
        with pytest.raises(
            TypeError, match="Normalization failed: Normalization failed"
        ):
