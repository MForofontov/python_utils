...
def test_throttle_failure():
    ...
    """
    assert sample_function() == "Function executed"
    with pytest.raises(RuntimeError, match="Function sample_function called too frequently. Rate limit: 1.0 seconds."):
        sample_function()
...
def test_throttle_with_logger(caplog):
    ...
    with caplog.at_level(logging.ERROR):
        with pytest.raises(RuntimeError, match="Function logged_function called too frequently. Rate limit: 1.0 seconds."):
            logged_function()
        assert "Function logged_function called too frequently. Rate limit: 1.0 seconds." in caplog.text
...
def test_invalid_rate_limit_type():
    ...
    """
    with pytest.raises(TypeError, match="rate_limit must be a positive float or an integer"):
    ...
    with caplog.at_level(logging.ERROR):
        with pytest.raises(TypeError, match="rate_limit must be a positive float or an integer"):
            ...
            def invalid_rate_limit_function_with_logger() -> None:
                pass
        assert "Type error in throttle decorator: rate_limit must be a positive float or an integer." in caplog.text
