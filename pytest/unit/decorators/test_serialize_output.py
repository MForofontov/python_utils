...
def test_serialize_output_with_var_args():
    ...
    assert function_with_var_args(1, "arg1", "arg2", kwarg1=1.0, kwarg2=2.0) == json.dumps({"args": ("arg1", "arg2"), "kwargs": {"kwarg1": 1.0, "kwarg2": 2.0}})
    ...
    with caplog.at_level(logging.ERROR):
        with pytest.raises(TypeError, match="format must be a string."):
            ...
            def invalid_format_type_with_logger_function() -> None:
                pass
        assert "Type error in serialize_output decorator: format must be a string." in caplog.text

def test_invalid_format():
    """
    Test case 6: Invalid format
    """
    with pytest.raises(ValueError, match="Unsupported format. Currently, only 'json' is supported."):
