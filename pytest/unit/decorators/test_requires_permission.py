...
def test_requires_permission_with_var_args():
    ...
    """
    @requires_permission('admin')
    def function_with_var_args(user_permissions: list[str], a: int, *args: str, **kwargs: float) -> str:
        return f"{a} - {args} - {kwargs}"
    
    assert function_with_var_args(['admin'], 1, "arg1", "arg2", kwarg1=1.0, kwarg2=2.0) == "1 - ('arg1', 'arg2') - {'kwarg1': 1.0, 'kwarg2': 2.0}"

def test_requires_permission_no_permissions():
    """
    Test case 5: User has no permissions
    """
    with pytest.raises(PermissionError, match="User does not have the required permission."):
        sample_function([])

def test_requires_permission_permission_not_list():
    """
    Test case 6: User permissions is not a list
    """
...
def test_requires_permission_permission_not_list_with_logger(caplog):
    """
    Test case 7: Logger functionality when type error occurs
    """
    ...
    with caplog.at_level(logging.ERROR):
        ...
        assert "Type error in logged_function: user_permissions must be a list." in caplog.text

def test_requires_permission_failure():
    """
    Test case 8: User does not have the required permission
    """
    with pytest.raises(PermissionError, match="User does not have the required permission."):
...
def test_requires_permission_with_logger(caplog):
    """
    Test case 9: Logger functionality when permission error occurs
    """
    @requires_permission('admin', logger=test_logger)
    def logged_function() -> str:
        return "Function executed"
    ...
    with caplog.at_level(logging.ERROR):
        with pytest.raises(PermissionError, match="User does not have the required permission. Required permission: 'admin', User permissions: ['user']"):
            logged_function(['user'])
        assert "User does not have the required permission. Required permission: 'admin', User permissions: ['user']" in caplog.text

def test_invalid_permission_type():
    """
    Test case 10: Invalid permission type
    """
