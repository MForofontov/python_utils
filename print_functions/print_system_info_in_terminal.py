...
def print_system_info_in_terminal() -> None:
    ...
    print_message(
        f"Total memory: {psutil.virtual_memory().total / (1024 * 1024):.2f} MB", "info"
    )
    print_message(
    ...
    print_message(
        f"System Uptime: {uptime_days} days, {uptime_hours} hours, {uptime_minutes} minutes, {uptime_remaining_seconds} seconds",
        "info",
    )
    print_message(separator, None)
