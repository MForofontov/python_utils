...
def analyze_predictions(
    ...
    if y_true.shape != y_pred.shape:
        raise ValueError(f"y_true and y_pred shapes must match: {y_true.shape} vs {y_pred.shape}")
    ...
    if task == 'regression':
        ...
        for cls, label in zip(classes, class_labels):
            ...
            if np.sum(mask) > 0:
                ...
                per_class_accuracy[label] = float(per_class_correct[label] / per_class_total[label])
