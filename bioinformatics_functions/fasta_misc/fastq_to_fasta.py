def fastq_to_fasta(fastq_str: str, min_quality: int | None = None) -> str:
    ...
    if min_quality is not None and not isinstance(min_quality, int):
        raise TypeError(f"min_quality must be an integer or None, got {type(min_quality).__name__}")
    ...
    if len(lines) % 4 != 0:
        raise ValueError(f"FASTQ format requires 4 lines per sequence, got {len(lines)} lines")
    ...
    for i in range(0, len(lines), 4):
        ...
        if not header_line.startswith('@'):
            raise ValueError(f"FASTQ header must start with '@', got: {header_line[:20]}")
        if not plus_line.startswith('+'):
            raise ValueError(f"FASTQ separator must start with '+', got: {plus_line[:20]}")
        if len(sequence_line) != len(quality_line):
            raise ValueError(f"Sequence and quality lengths must match: {len(sequence_line)} vs {len(quality_line)}")
