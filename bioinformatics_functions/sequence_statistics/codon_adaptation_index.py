...
def codon_adaptation_index(seq: str, reference_weights: dict[str, float] | None = None) -> float:
    """
    ...
    if reference_weights is not None and not isinstance(reference_weights, dict):
        raise TypeError(f"reference_weights must be a dict or None, got {type(reference_weights).__name__}")
