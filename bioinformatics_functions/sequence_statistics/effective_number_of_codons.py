...
def effective_number_of_codons(seq: str) -> float:
    ...
    def calculate_F(aa_list: list[str], aa_to_codons: dict[str, list[str]], codon_counts: dict[str, int]) -> float:
        ...
        for aa in aa_list:
            ...
            if total_aa > 0:
                homozygosity = sum((codon_counts.get(codon, 0) / total_aa) ** 2 for codon in codons)
