...
def find_cpg_islands(seq: str, window: int = 200, min_gc: float = 0.5, 
                     min_obs_exp: float = 0.6) -> List[Tuple[int, int]]:
    """
    ...
    if not isinstance(min_obs_exp, (int, float)):
        raise TypeError(f"min_obs_exp must be a number, got {type(min_obs_exp).__name__}")
