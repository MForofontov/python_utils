def remove_low_complexity_regions(seq: str, window_size: int = 10, complexity_threshold: float = 1.5, replace_with: str = 'N') -> str:
    """
    ...
    if not isinstance(complexity_threshold, (int, float)):
        raise TypeError(f"complexity_threshold must be a number, got {type(complexity_threshold).__name__}")
    if not isinstance(replace_with, str):
        raise TypeError(f"replace_with must be a string, got {type(replace_with).__name__}")
    
    if window_size < 2:
        raise ValueError(f"window_size must be at least 2, got {window_size}")
    if complexity_threshold < 0 or complexity_threshold > 2:
        raise ValueError(f"complexity_threshold must be between 0 and 2, got {complexity_threshold}")
    if len(replace_with) != 1:
        raise ValueError(f"replace_with must be a single character, got '{replace_with}'")
